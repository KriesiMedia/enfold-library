	...
	abstract class aviaShortcodeTemplate
	{
		...
		*/
		...
		public function shortcode_handler_prepare( $atts, $content = '', $shortcodename = '', $fake = false )
		{
						...
						if( $current_post instanceof WP_Post )
						{
							...
							else 
							{
								...
								if( false !== apply_filters( 'avf_shortcode_handler_prepare_no_tree', false, $current_post->ID, $current_post ) )
								{
									...
									$content = ShortcodeHelper::clean_up_shortcode( trim( $current_post->post_content ), 'balance_only' );
			...
			if( ( ! $current_post instanceof WP_Post ) || ( ! in_array( $current_post->post_type, Avia_Builder()->get_supported_post_types() ) ) || ( 'active' != Avia_Builder()->get_alb_builder_status( $current_post->ID ) ) )
			{
				$meta['el_class'] = '';
				
				/**
				 * For non ALB pages we ignore index ( 0 makes problem with e.g. sections )
				 */
				$meta['index'] = -1;
			}
