...
if ( ! class_exists( 'avia_magazine' ) )
{
	class avia_magazine
	{
		...
		public function query_entries( $params = array(), $return = false )
		{
			...
			/**
			 * Fix: to use post date instead of event start date
			 */
			add_filter( 'tribe_query_can_inject_date_field', array( $this, 'tribe_can_inject_date_field' ), 10, 2 );
			
			$entries = new WP_Query( $query );
			
			/**
			 * Fix: remove filter again
			 */
			remove_filter( 'tribe_query_can_inject_date_field', array( $this, 'tribe_can_inject_date_field' ), 10 );
		...
		* @param boolean  $can_inject Whether the date field can be injected
		 ...
		 * @since tribe 4.5.8
		 */
		public function tribe_can_inject_date_field( $can_inject, $query )
		{
			return false;
		}
