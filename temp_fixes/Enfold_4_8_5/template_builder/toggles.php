		...
		protected function get_element_styles( array $args )
		{
			...
			if( 'custom' == $atts['colors'] )
			{
				...
				if( ! empty( $atts['font_color'] ) )
				{
					...
					$element_styling->add_classes( 'item_titleClass', 'hasCustomColor' );
					
				}
				else
				{
					$element_styling->add_styles( 'toggle-icon', array( 
															'color'			=> $atts['border_color'],
															'border-color'	=> $atts['border_color'] 
														) );
					
					$element_styling->add_classes( 'item_inherit', 'av-inherit-font-color' );
				}
			...
			//	override dynamic stylesheet setting
			$element_styling->add_styles( 'toggle-hover', $cols );
			...
			$selectors = array(
						'container'			=> ".togglecontainer.{$element_id}",
						'toggle'			=> ".togglecontainer.{$element_id} p.toggler",
						'toggle-current'	=> ".togglecontainer.{$element_id} p.toggler.activeTitle",
						'toggle-hover'		=> ".togglecontainer.{$element_id} p.toggler:hover",
						'toggle-hover-not'	=> ".togglecontainer.{$element_id} p.toggler:not(.activeTitle):hover",
						'toggle-icon'		=> "#top .togglecontainer.{$element_id} p.toggler .toggle_icon",
						'toggle-content'	=> ".togglecontainer.{$element_id} .toggle_wrap .toggle_content",
					);
