	...
	abstract class aviaShortcodeTemplate
	{
		...
		*/
		...
		public function shortcode_handler_prepare( $atts, $content = '', $shortcodename = '', $fake = false )
		{
			...
			if( ! Avia_Builder()->in_text_to_preview_mode() )
			{
				...
				if( ! is_array( ShortcodeHelper::$tree ) || empty( ShortcodeHelper::$tree ) )
				{
					...
					if( ! is_admin() )
					{
						...
						/**
						 * In front we try to init if we have a post - else we return nothing
						 */
			...
			{
				/**
				 * Fallback to avoid undefined index notice - as we have no shortcode tree
				 */
				if( ! isset( $meta['index'] ) )
				{
			...
			{
				/**
				 * Add a logfile output
				 */
				if( ! Avia_Builder()->in_text_to_preview_mode() && ( ! isset( $meta['index'] ) ) )
				{
