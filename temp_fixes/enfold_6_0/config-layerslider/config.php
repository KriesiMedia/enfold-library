...
if( ! class_exists( 'Avia_Config_LayerSlider', false ) )
{
	class Avia_Config_LayerSlider
	{
		...
		public function find_layersliders( $names_only = false )
		{
			...
			/**
			 * Change the order by string for the selectbox in backend canvas
			 * Valid database fields could be:
			 *			- id
			 *			- name
			 *			- slug
			 *			- date_c
			 *			- date_m
			 *
			 * There is no check for a valid value !!!!
			 *
			 * @since x.x.x
			 * @param string $order_by
			 * @return string
			 */
			$order_by = apply_filters( 'avf_layerslider_order_by_select_alb', 'date_c ASC' );
		...
		public function get_default_slider( $id = 0 )
		{
			...
			if( $id > 0 )
			{
				$slider = $wpdb->get_row(
										"SELECT * FROM {$table_name}
										WHERE id = {$id}  AND flag_hidden = '0' AND flag_deleted = '0'
										LIMIT 1",
									ARRAY_A );
			}
