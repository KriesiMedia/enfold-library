...
if( ! class_exists( __NAMESPACE__ . '\aviaPopupTemplatesResppnsive', false ) )
{
	class aviaPopupTemplatesResponsive extends \aviaBuilder\base\aviaPopupTemplatesCallback
	{
		...
		protected function font_sizes_icon_switcher( array $element )
		{
			...
			{
				/**
				 * Handle situation when 'medium' not set
				 * https://kriesi.at/support/topic/undefined-array-key-medium-in-class-popup-templates-responsive-php/
				 *
				 * @since 5.7.2
				 */
				if( isset( $id_sizes['medium'] ) )
				{
					$id_sizes['desktop'] = str_replace( 'av-medium-', 'av-desktop-', $id_sizes['medium'] );
				}
				else if( isset( $id_sizes['small'] ) )
				{
					$id_sizes['desktop'] = str_replace( 'av-small-', 'av-desktop-', $id_sizes['small'] );
				}
				else if( isset( $id_sizes['mini'] ) )
				{
					$id_sizes['desktop'] = str_replace( 'av-mini-', 'av-desktop-', $id_sizes['mini'] );
				}
				else if( isset( $id_sizes['default'] ) )
				{
					$id_sizes['desktop'] = "av-desktop-{$id_sizes['default']}";
				}
