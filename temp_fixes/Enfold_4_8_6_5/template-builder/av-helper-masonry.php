...
if( ! class_exists( 'avia_masonry' ) )
{
	class avia_masonry
	{
		...
		public function __construct( $atts = array() )
		{
			...
			$this->atts = shortcode_atts( avia_masonry::default_args(), $atts, 'av_masonry_entries' );
			...
			if( 'no scaling' == $this->atts['image_size'] )
			{
				$this->atts['image_size'] = 'full';
			}
		 ...
		 *
		 * @since < 4.0
		 * @since 4.8.4			added $element_styling, $sc_context
		 * @param \aviaBuilder\base\aviaElementStylingBase|null $element_styling
		 * @param string $element_id
		 * @param aviaShortcodeTemplate $sc_context
		 * @return string
		 */
		public function html( \aviaBuilder\base\aviaElementStylingBase $element_styling = null, $element_id = '', aviaShortcodeTemplate $sc_context = null )
		{
			if( empty( $this->loop ) )
			{
				return '';
			...
			//	fallback - code no longer supported since 4.8.4
			if( is_null( $element_styling ) )
			{
				return $this->html_no_post_css( $sc_context );
			}
			...
			$output  = '';
			$output .= $style_tag;
			$output .= "<div id='{$id_el}' class='{$container_class} {$custom_class} {$av_display_classes} {$av_column_classes}'>";
			...
			foreach( $this->loop as $index => $entry )
			{
				...
				if( ! empty( $attachment ) )
				{
					...
					switch( $this->atts['lightbox_text'] )
					{
						case 'caption':
					...
					if( isset( $attachment[0] ) )
					{
						if( $size == 'flex' )
						{
							...
							$img_title = ( 'active' == $this->atts['container_links'] ) ? $lightbox_title : $title;

							$img_html = '<img src="'. $attachment[0] . '" title="' . $img_title . '" alt="' . $alt . '" />';
							$img_html = Av_Responsive_Images()->make_image_responsive( $img_html, $thumb_ID, $this->atts['lazy_loading'] );
				...
				if( $this->atts['caption_elements'] != 'none' || ! empty( $text_add ) )
				{
					...
					if( strpos( $this->atts['caption_elements'], 'title' ) !== false )
					{
						 ...
						 * @return string
						 */
						$the_title = apply_filters( 'avf_masonry_entry_title', $the_title, $entry, $this->atts );
		...
		/**
		 * Added for a fallback for function html() if calling element does not support post css files
		 *
		 * @since 4.8.4
		 * @deprecated 4.8.4
		 ...
		 * @return string
		 */
		public function html_no_post_css( aviaShortcodeTemplate $sc_context = null )
		{
			_deprecated_function( 'avia_masonry::html()', '4.8.4', 'Calling this function without post css support will be deprecated in a future version' );
			...
			foreach( $this->loop as $index => $entry )
			{
				...
				if( $post_type == 'attachment' && strpos( $html_tags[0], 'a href=' ) !== false )
				{
					...
					{
						case 'caption':
							$lightbox_title = ( '' != $entry['content'] ) ? $entry['content'] : $title;
							break;
						case 'description':
							$lightbox_title = ( '' != $entry['image_content'] ) ? $entry['image_content'] : $title;
							break;
						case 'no_text':
							$lightbox_title = '';
					}

					$linktitle = 'title="' . esc_attr( $lightbox_title ) . '" alt="' . esc_attr( $alt ) . '" ';
				}
				else if( strpos( $html_tags[0], 'a href=' ) !== false )
				{
					$display = empty( $title ) ? $the_title : $title;
			...
			foreach( $this->entries->posts as $key => $entry )
			{
				...
				switch( $entry->post_type )
				{
					...
					case 'attachment':
						 ...
						 * @return string
						 */
						$lightbox_img_size = apply_filters( 'avf_alb_lightbox_image_size', $lightbox_img_size, 'avia_masonry', $entry, $this->atts );
						...
						{
							if( 'no_links' == $this->atts['container_links'] )
							{
								$this->loop[ $key ]['url'] = '';
							}
							else if( '' == $this->atts['container_links'] )
							{
								$this->loop[ $key ]['url'] = $custom_url;
							}
							else
				...
				if( $this->atts['container_links'] !== 'no_links' )
				{
					if( ! empty( $this->atts['container_links'] ) )
					{
						$set_link = true;
					}
					else
		 ...
		 * @since 4.8.4
		 * @param array $result
		 * @param aviaShortcodeTemplate $sc_context				context of calling shortcode
		 * @return array
		 */
		public function get_element_styles( array $result, aviaShortcodeTemplate $sc_context )
		{
			extract( $result );
