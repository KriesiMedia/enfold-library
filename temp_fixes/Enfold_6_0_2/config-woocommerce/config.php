...
if( ! function_exists( 'avia_woocommerce_ignore_duplicate_post_types' ) )
{
	/**
	 * WooCommerce has its own duplicate product logic - remove Enfold link
	 *
	 * @since 5.6.9
	 * @param array $post_types
	 * @return array
	 */
	function avia_woocommerce_ignore_duplicate_post_types( $post_types = [] )
	{
	...
	add_filter( 'avf_ignore_duplicate_post_types', 'avia_woocommerce_ignore_duplicate_post_types', 10, 1 );
}
...
if( ! function_exists( 'avia_woocommerce_page_content_post_types' ) )
{
	/**
	 * Add post type 'product' to display CPT in selectbox for ALB element custom layout
	 *
	 * @since 6.0
	 * @param array $post_types
	 * @return array
	 */
	function avia_woocommerce_page_content_post_types( array $post_types )
	{
	...
	add_filter( 'avf_page_content_post_types', 'avia_woocommerce_page_content_post_types', 10, 1 );
}
		...
		if( is_product_category() )
		{
			...
			if( isset( $wp_query->query_vars['taxonomy'] ) )
			{
				...
				if( ! empty( $term->term_id ) )
				{
					...
					//	display default WC thumbnail if banner image not selected
					if( ! empty( $attachment_id ) && empty( $style ) )
					{
						$args = [
								'title'	=> trim( strip_tags(get_post_field( 'post_title', $attachment_id ) ) ),
								'class' => "category_thumb attachment-{$image_size} size-{$image_size}"
							];

						$image = wp_get_attachment_image( $attachment_id, $image_size, false, $args );
						if( $image )
						{
							echo "<div class='page-thumb'>{$image}</div>";
						}
...
if( ! function_exists( 'avia_woocommerce_shop_banner' ) )
{
	...
	function avia_woocommerce_shop_banner()
	{
		...
		$banner = avia_get_option( 'shop_banner', '' );

		if( empty( $banner ) || isset( $avia_config['woo-banner'] ) || isset( $avia_config['woo-banner-cat'] ) )
		{
			return;
		}

		$title_bar = avia_get_option( 'header_title_bar', 'hidden_title_bar' );
		$location = avia_get_option( 'shop_banner_location', '' );

		switch( current_action() )
		{
			case 'ava_after_main_container':
				if( 'av-shop-banner-below' == $location )
				{
					if( 'hidden_title_bar' != $title_bar )
					{
						return;
		...
		if( is_shop() || ( is_product_category() && avia_get_option( 'shop_banner_global' ) == 'shop_banner_global' ) && ! isset( $avia_config['woo-banner'] ) )
		{
			...
			if( isset( $options['shop_banner'] )  && ( false !== strpos( $options['shop_banner'], 'av-active-shop-banner' ) ) )
			{
				...
				echo avia_woocommerce_parallax_banner( $bg, $overlay, $opacity, $description, $font, $options['shop_banner'] );
	...
	add_action( 'ava_after_main_container', 'avia_woocommerce_shop_banner', 12 );
	add_action( 'ava_after_main_title', 'avia_woocommerce_shop_banner', 12 );
}
...
if( ! function_exists( 'avia_woocommerce_parallax_banner' ) )
{
	...
	function avia_woocommerce_parallax_banner( $bg, $overlay, $opacity, $description, $font, $effect = '' )
	{
		...
		if( is_numeric( $bg ) )
		{
			if( ! $is_responsive )
			{
				$img_info = wp_get_attachment_image_src( $bg, $image_size );

				if( is_array( $img_info ) && $img_info[0] != '' )
				{
					$bg = $img_info[0];
					$bg_img = $bg;
				}
			...
			else
			{
				...
				$img_tag = wp_get_attachment_image( $bg, $image_size, false, $args );
			}
		}
		else
		{
			...
			if( $is_responsive )
			{
				...
				$size = [];
				preg_match('/-\d+[Xx]\d+\./', $bg, $size );

				if( $att_id <= 0 )
				{
					//	fallback - we set an unresponsive default image
					$img_tag = "<img class='avia_image' src='{$bg}' alt='{$title_attr}' title='{$title_attr}' decoding='async' {$markup_url} />";
				}
				else
				{
					if( ! empty( $size ) )
					{
						$size = str_replace( 'X', 'x', $size[0] );
						$size = explode( 'x', $size );
						$w = str_replace( '-', '', $size[0] );
						$h = str_replace( '.', '', $size[1] );

						$imagedata = wp_get_attachment_metadata( $att_id );

						if( is_array( $imagedata ) && ! empty( $imagedata['sizes'] ) )
						{
							foreach( $imagedata['sizes'] as $_size => $data )
							{
								if( (int) $data['width'] === (int) $w && (int) $data['height'] === (int) $h )
								{
									$image_size = $_size;
									break;
								}
					...
					$args = [
							'title'	=> trim( strip_tags(get_post_field( 'post_title', $att_id ) ) )
						];

					$img_tag = wp_get_attachment_image( $att_id, $image_size, false, $args );
				}
			}
		}
		...
		$parallax_class = $is_parallax ? 'av-parallax' : '';
		$responsive_class = $is_responsive ? 'av-responsive-banner avia-section-small' : 'avia-section-large';
		$desc_tag = $is_responsive ? 'div' : 'h1';
