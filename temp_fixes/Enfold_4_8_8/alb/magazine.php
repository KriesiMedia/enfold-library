...
if( ! class_exists( 'avia_sc_magazine' ) )
{
	class avia_sc_magazine extends aviaShortcodeTemplate
	{
		...
		protected function get_element_styles( array $args )
		{
			...
			$default = avia_magazine::get_defaults( $this->get_default_sc_args() );
			...
			$atts['class'] = ! empty( $meta['el_class'] ) ? $meta['el_class'] : '';
			$atts['custom_markup'] = ! empty( $meta['custom_markup'] ) ? $meta['custom_markup'] : '';
			...
			{
				$this->obj_magazines[ $element_id ] = new avia_magazine( $atts );
			}
			...
			$result = $magazine->get_element_styles( $result, $this );
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			$html = $magazine->html( $element_styling, $element_id, $this );
...
if( ! class_exists( 'avia_magazine' ) )
{
	class avia_magazine
	{
		...
		{
			$this->atts = shortcode_atts( avia_magazine::get_defaults( $atts ), $atts, 'av_magazine' );
			...
			$this->atts['meta_author'] = $this->atts['meta_author'] === 'aviaTBmeta_author' ? true : false;
			$this->atts['meta_cats'] = $this->atts['meta_cats'] === 'aviaTBmeta_cats' ? true : false;
			$this->atts['meta_tags'] = $this->atts['meta_tags'] === 'aviaTBmeta_tags' ? true : false;
		...
		static public function get_defaults( array $defaults = array() )
		{
			...
			return array_merge( $default, $defaults );
		}
		...
		public function get_element_styles( array $result, aviaShortcodeTemplate $sc_context )
		{
			...
			if( ! empty( $this->atts['first_big_pos'] ) )
			{
				$classes[] = 'av-magazine-hero-' . $this->atts['first_big_pos'];
			}
			...
			$element_styling->add_classes( 'container', $this->atts['class'] );
			...
			if( $this->atts['heading_color'] != 'theme-color' )
			{
				...
				{
					$element_styling->add_styles( 'heading', array( 'color' => $this->atts['heading_custom_color'] ) );
				}
			}
			...
			$selectors = array(
							...
							'heading'		=> "#top .av-magazine.{$element_id} a.av-magazine-top-heading",
				);
		 ...
		 *
		 * @since < 4.0
		 * @since 4.8.8			added $element_styling, $sc_context
		...
		public function html( \aviaBuilder\base\aviaElementStylingBase $element_styling = null, $element_id = '', aviaShortcodeTemplate $sc_context = null )
		{
			...
			$output  = '';
			$output .= $style_tag;
			$output .= "<div {$id} class='{$container_class} {$this->atts['class']}' >";
