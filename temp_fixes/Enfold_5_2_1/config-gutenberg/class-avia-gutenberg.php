...
if( ! class_exists( 'Avia_Gutenberg' ) )
{
	...
	class Avia_Gutenberg
	{
		...
		public function handler_wp_register_scripts()
		{
			...
			wp_register_style( 'avia_gutenberg_css', "{$template_url}/config-gutenberg/css/avia_gutenberg{$min_css}.css", array( 'avia-modal-style', 'avia-builder-style' ), $vn );
			Avia_Builder()->add_registered_admin_style( 'avia_gutenberg_css' );

			wp_register_script( 'avia_gutenberg_script', "{$template_url}/config-gutenberg/js/avia_gutenberg{$min_js}.js", array( 'avia_builder_js' ), $vn, true );
		...
		{
			/**
			 * Disable fixed tab container on fullscreen mode - beta
			 *
			 * @since 5.1
			 */
			$expand = current_theme_supports( 'avia_block_editor_disable_fixed_tab' ) ? '' : ' avia-block-editor-expand';
...
//			if( ( ! in_array( $obj_post_type->name, Avia_Builder()->get_supported_post_types() ) ) || ( ! $this->can_use_block_editor( $obj_post_type->name, true ) ) )
		 ...
		 *
		 * @since 4.5.1
		 * @since 5.3						removed array from $post_states
		...
		public function handler_display_post_states( $post_states, $post = null )
		{
			 ...
			 *
			 * @since 4.7.4.1			WP_Post check
			 * @since 5.3				array check
			 */
