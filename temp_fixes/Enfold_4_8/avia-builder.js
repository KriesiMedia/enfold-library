	...
	$.AviaBuilder = function () 
	{
		...
		*/
		set_up: function()
		{
			...
			//	Fix a problem to force open ALB on new alb_elements post type
...
//									console.log( 'avia_alb_shortcode_buttons_order error: ', errorObj );
		...
		*/
		switch_layout_mode: function(event)
		{
			...
			if( self.switch_button.is( '.av-builer-button-disabled' ) ) 
			{
				return false;
			}
		...
		*/
		...
		/**
		 * Make sure a shortcode ends witn 2 \n because WP converts 1 \n to <br> in filter "the_content" which breaks output.
		 * 
		 * @param {string} shortcode
		 * @returns string
		 */
		ensureLineBreaks: function( shortcode )
		{
		...
		*/
		updateTextarea: function( scope, builder_selector, clean_data_selector )
		{	
		    ...
		    if(!scope) 
		    {
		        ...
		        $( builderSelector ).find('.avia_layout_section').each(function()
		        {
					...
					single_section_tabs.each( function(){
									...
									obj.updateTextarea( cells, builder_selector, clean_data_selector );
							});
...
//		        	}
		        	...
		        	{
                    	obj.updateTextarea( coll_in_cell, builder_selector, clean_data_selector );
		        	}
		        	
		        	if(col_in_section.length)
		        	{
                    	obj.updateTextarea( col_in_section, builder_selector, clean_data_selector );
		        	}
		        });
			...
			for (var i = 0; i < content_fields.length; i++) 
			{
				...
				//	make sure we have 2 line breaks otherwise WP will add a <br> tag which breaks layout
				content += this.ensureLineBreaks( currentContent );
			}
