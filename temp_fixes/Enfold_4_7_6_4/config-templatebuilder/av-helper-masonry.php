			...
			{ 	
				...
				switch( $entry->post_type )
				{
					...
					case 'attachment':
						...
						$img_url = wp_get_attachment_image_src( $id, apply_filters( 'avf_avia_builder_masonry_lightbox_img_size', 'large' ) );
						$img_url = is_array( $img_url ) ? reset( $img_url ) : '';
						
						/**
						 * Allow a fallback to behaviour up to 4.7.6.3 where a set $custom_url was used regardess of option selection.
						 * This changed in 4.7.6.4 where it is possible to use a custom_url and lightbox for an image.
						 * 
						 * @since 4.8
						 * @param boolean
						 * @param array $this->loop
				...
				if( $set_link )
				{
					$this->loop[ $key ]['html_tags'] = array( 'a href="' . $this->loop[ $key ]['url'] . '" ' . $aria_label_href . $target, 'a' ); //opening and closing tag for the masonry container
				}
				else
				{
