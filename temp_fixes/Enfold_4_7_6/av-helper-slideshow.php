...
if ( ! class_exists( 'avia_slideshow' ) )
{
	class avia_slideshow
	{
		...
		public function html()
		{
			...
			if( ! empty( $this->config['control_layout'] ) ) 
			{
				$extraClass .= ' ' . $this->config['control_layout'];
			}
			...
			{
				$html .= $this->config['caption_override'];
			}
		...
		protected function default_slide()
		{
			...
			foreach( $this->id_array as $id )
			{
				if( isset( $this->slides[ $id ] ) )
				{
					...
					$img_tag = Av_Responsive_Images()->make_image_responsive( $img_tag, $slide->ID );
